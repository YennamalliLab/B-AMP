...
const generateDockedPepsSet = () => {
	const dockedPepsList = [
		26, 49, 50, 51, 72, 88, 109, 113, 115, 167, 168, 187, 696, 782, 993, 994, 995, 996, 1005,
		1086, 1087, 1088, 1089, 1218, 1219, 1245, 1326, 1437, 2442, 2533, 2534, 3052, 3059, 3219,
		3239, 3240, 3292, 3296, 3297, 3298, 3308, 3309, 3349, 3366, 3461, 3469, 3470, 3475, 3983,
		4020, 4021, 4583, 4674, 4707, 4708, 4709, 4710, 4785, 4819, 4844, 4886, 4931, 4932, 4933,
		4934, 5235, 5236, 5238, 5240, 5241, 5242, 5243, 5244, 5245, 5363, 5481, 5482, 5483, 5484,
		5485, 5486, 5487, 5488, 5489, 5490, 5491, 5492, 5493, 5494, 5495, 5496, 5497, 5498, 5499,
		5500, 5501, 5502, 5505, 5508, 5509,
	];
...
const search = (query, env) => {
	...
	const addToResultSet = (resultItem) => {
		...
		if (env === "ag+") {
			...
			const pepActivity = PEP_TO_ACTIVITY_AND_NAME[resultItem["pepID"]][0];
			if (!(pepActivity === 2 || pepActivity === 3)) return;
	...
	for (const queryItem of queryItems) {
		...
		if (queryItem.startsWith("DRAMP") && queryItem.length > 5) {
			...
			if(!["5564","5565","5566","5567","5568","5569","5570","5571","5800","5801","5802","5803","5804","5805","5806","5807","5808","5815","5869"].includes(resultItem['pepID']))
		...
		for (const queryWord of queryItem.split(" ")) {
			// Full-text search
			const peps = TEXT_TO_PEP[queryWord.toLowerCase()];

			if (peps === undefined) continue;

			for (const pep of peps) {
				if(!["5564","5565","5566","5567","5568","5569","5570","5571","5800","5801","5802","5803","5804","5805","5806","5807","5808","5815","5869"].includes(pep)){
					const resultItem = {
						pepID: pep,
